...
gcloud container node-pools create $STANDARD_POOL --cluster $CLUSTER_NAME --node-taints pool=$STANDARD_POOL:NoSchedule --enable-autoscaling --num-nodes 1 --min-nodes 1 --max-nodes 10 --machine-type e2-standard-4 --disk-size=20GB # changed because already committed to this machine type
...
gcloud container node-pools create $MESH_POOL --cluster $CLUSTER_NAME --node-taints pool=$MESH_POOL:NoSchedule --num-nodes 1 --preemptible --enable-autoscaling --min-nodes 1 --max-nodes 100 --disk-size=20GB --machine-type e2-standard-4 # changed because already committed to this machine type
